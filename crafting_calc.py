...
def get_profit(item_arg, text_box=None):
    return item_arg.get_price() - get_crafting_cost(item_arg, text_box)


def get_gph(item_arg, text_box=None):
    """
    Derive gil/hour.
    """
    if len(item_arg.sales_data) > 0 and len(item_arg.stock_data) > 0:
        ...
        for x in item_arg.sales_data:
            matching_stock = match_stock(x, copy_stock_data)
        ...
        if len(deltas) > 0:
            ...
            return math.floor(get_profit(item_arg, text_box) / (avg_time.days * 24 + avg_time.seconds / 3600))


def get_crafts(num, text_box=None):
    """
        ...
        if x.stock <= 0:
            ...
            if x.sales > 0:
                ...
                if gph is not None:
                    ...
                    x.profit = get_profit(x, text_box)
            else:
    ...
    for x in item_to_check.reagents.items():
        ...
        if mat.craftable is False:
            ...
            if len(mat.reagents) > 0:
                crafting_cost += get_crafting_cost(mat, text_box) * float(x[1])
            else:
...
def match_stock(sale_time, stock_list):
    for x in stock_list:
        if x < sale_time:
